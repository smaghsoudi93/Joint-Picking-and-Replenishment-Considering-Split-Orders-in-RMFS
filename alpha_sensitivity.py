...
# ===========================================================================
# ALPHA SENSITIVITY SWEEP  (Fig. 3, Section 5.3)
# ===========================================================================
# Runs the full proposed pipeline (order-aware Phase 1 + VNS) once per value
# of the coordination weight alpha, on the instance in 'Data2 - new.xlsx'.
# For each alpha it records: number of extra splits, pod visits, unused
# capacity, decision cost (alpha*splits + visits) and total objective, then
# writes 'alpha_sensitivity_results.csv' and renders the Fig. 3 plots.
#
# Optional: to overlay the EXACT model, create 'alpha_sensitivity_exact.csv'
# by hand with the same columns (alpha,splits,visits,unused,decision,total)
# filled from the OPL runs; the plot script picks it up automatically.
...
# ---- read the remaining parameters exactly like the main driver ----
for row in range(1, sheet.max_row + 1):
    for col in range(1, sheet.max_column + 1):
        if sheet.cell(row=row, column=col).value == 'num_picking_stations':
            num_picking_stations_row = row
...
VISIT_LB = 2 * (-(-total_demand_all // Pods_capacity))
...
for a in ALPHA_VALUES:
    print("\n" + "=" * 70)
    print(f"ALPHA SWEEP: alpha = {a}")
    print("=" * 70)
    globals()['ALPHA_COORDINATION'] = a
    # identical seed per run -> differences are due to alpha only
    random.seed(SEED)
    np.random.seed(SEED)

    t0 = time.time()
    best_clusters, best_cost = variable_neighborhood_search(
        MAX_ITERATIONS, num_picking_stations, Demand, group_capacity,
        patience=PATIENCE, visit_lb=VISIT_LB)
    t1 = time.time()

    _, _, splits, unused = derive_solution_components(
        best_clusters, num_picking_stations, Demand, group_capacity)
    visits = int(round(best_cost - a * splits - UNUSED_WEIGHT * unused))
    decision = a * splits + visits
    results.append({'alpha': a, 'splits': splits, 'visits': visits,
                    'unused': unused, 'decision': decision,
                    'total': best_cost, 'time_s': round(t1 - t0, 1)})
    print(f"[sweep] alpha={a}: splits={splits} visits={visits} "
          f"decision={decision} total={best_cost} ({t1-t0:.1f}s)")

# restore default
globals()['ALPHA_COORDINATION'] = 10

with open('alpha_sensitivity_results.csv', 'w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=['alpha', 'splits', 'visits', 'unused',
                                      'decision', 'total', 'time_s'])
    w.writeheader()
    w.writerows(results)
print("\nwrote alpha_sensitivity_results.csv")
print(f"{'alpha':>6} {'splits':>7} {'visits':>7} {'decision':>9} {'total':>8} {'time':>7}")
for r in results:
    print(f"{r['alpha']:>6} {r['splits']:>7} {r['visits']:>7} "
          f"{r['decision']:>9} {r['total']:>8} {r['time_s']:>6}s")
...
try:
    ...
    heu = load('alpha_sensitivity_results.csv')
    exa = load('alpha_sensitivity_exact.csv')   # optional overlay (OPL runs)

    plt.rcParams.update({'font.family': 'serif', 'font.size': 11,
                         'axes.grid': True, 'grid.alpha': 0.35,
                         'grid.linestyle': ':'})

    # Fig 3a: number of split orders vs alpha
    fig, ax = plt.subplots(figsize=(5.2, 3.6))
    ax.plot(heu['alpha'], heu['splits'], marker='o', color='black',
            label='Proposed algorithm')
    if exa:
        ax.plot(exa['alpha'], exa['splits'], marker='s', linestyle='--',
                color='gray', label='Exact model (CPLEX)')
        ax.legend(frameon=False)
    ax.set_xlabel(r'Coordination weight $\alpha$')
    ax.set_ylabel('Number of extra splits')
    ax.set_xticks(heu['alpha'])
    fig.tight_layout()
    fig.savefig('Fig3a_splits_vs_alpha.png', dpi=300)
    fig.savefig('Fig3a_splits_vs_alpha.pdf')

    # Fig 3b: decision cost and pod visits vs alpha
    fig, ax = plt.subplots(figsize=(5.2, 3.6))
    ax.plot(heu['alpha'], heu['visits'], marker='o', color='black',
            label='Pod visits ($\\Sigma x + \\Sigma v$)')
    ax.plot(heu['alpha'], heu['decision'], marker='^', color='dimgray',
            label='Decision cost (visits + $\\alpha\\cdot$splits)')
    if exa:
        ax.plot(exa['alpha'], exa['visits'], marker='s', linestyle='--',
                color='gray', label='Pod visits (exact)')
    ax.set_xlabel(r'Coordination weight $\alpha$')
    ax.set_ylabel('Cost')
    ax.set_xticks(heu['alpha'])
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig('Fig3b_cost_vs_alpha.png', dpi=300)
    fig.savefig('Fig3b_cost_vs_alpha.pdf')
    print("wrote Fig3a/Fig3b (png + pdf)")
except ImportError:
    print("matplotlib not installed - run: pip install matplotlib  (plots skipped)")
