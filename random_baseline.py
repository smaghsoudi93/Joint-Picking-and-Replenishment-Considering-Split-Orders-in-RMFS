...
# random_baseline.py
# ---------------------------------------------------------------------------
# RANDOM-ASSIGNMENT BASELINE for the RMFS paper (referee/supervisor request:
# quantify the value of the co-occurrence-based Phase 1 against a random
# assignment of order items to picking stations).
#
# Every (order, item) pair is assigned to a picking station drawn uniformly at
# random among those with enough remaining capacity, i.e. the assignment is
# feasible but carries no demand-affinity or coordination information.
# Because the outcome is stochastic, the instance is solved REPLICATES times
# with different seeds and the mean / best / worst objective are reported.
#
# Everything downstream (wave planning, pod and replenishment decisions) is
# identical to the proposed method, so objectives are directly comparable.
#
# Usage: place next to 'Data2 - new.xlsx' and run.
# ===========================================================================
...
def random_assignment(Demand, num_picking_stations, station_capacity, rng):
    """Assign each (order, item) pair to a uniformly random picking station
    among those that still have room. Items are visited in random order so
    that no positional bias (as in first-fit) is introduced. Item-level
    integrality is respected, exactly as required by constraints (3.4)."""
    num_orders = len(Demand)
    num_skus = len(Demand[0])

    items = [(o, i, int(Demand[o][i]))
             for o in range(num_orders) for i in range(num_skus)
             if Demand[o][i] and Demand[o][i] > 0]
    rng.shuffle(items)
    # largest items first inside the shuffled order would bias the packing;
    # instead we keep the random order but retry the whole assignment if it
    # gets stuck, which keeps the baseline purely random yet feasible.
    for attempt in range(200):
        clusters = [[] for _ in range(num_picking_stations)]
        load = [0] * num_picking_stations
        ok = True
        order_of_items = items[:]
        rng.shuffle(order_of_items)
        for (o, i, qty) in order_of_items:
            choices = [s for s in range(num_picking_stations)
                       if load[s] + qty <= station_capacity]
            if not choices:
                ok = False
                break
            s = rng.choice(choices)
            clusters[s].append((o, i, qty))
            load[s] += qty
        if ok:
            return clusters
    print("[random] could not build a feasible random assignment in 200 attempts")
    return None
...
REPLICATES = 10          # independent random assignments
BASE_SEED = 42

print("=" * 72)
print("RANDOM-ASSIGNMENT BASELINE  (uniform random, no affinity, no coordination)")
print(f"replications = {REPLICATES}")
print("=" * 72)
...
for rep in range(REPLICATES):
    ...
    clusters = random_assignment(Demand, num_picking_stations, group_capacity, rng)
    if clusters is None:
        continue
    y, yy, coordination_cost, total_unused_capacity = derive_solution_components(
        clusters, num_picking_stations, Demand, group_capacity)
    total_cost = evaluate_clusters(clusters, yy, y,
    ...
    visits = int(round(total_cost - ALPHA_COORDINATION * coordination_cost
                       - UNUSED_WEIGHT * total_unused_capacity))
    results.append(dict(rep=rep, splits=coordination_cost,
                        split_orders=n_split_orders, visits=visits,
                        unused=total_unused_capacity, total=total_cost))
    print(f"[random] rep {rep}: extra splits = {coordination_cost:>3} | "
          f"orders split = {n_split_orders:>3}/{n_orders} | visits = {visits:>3} | "
          f"total = {total_cost}")

t_end = time.time()
if results:
    tot = [r['total'] for r in results]
    spl = [r['splits'] for r in results]
    vis = [r['visits'] for r in results]
    mean = sum(tot) / len(tot)
    var = sum((v - mean) ** 2 for v in tot) / len(tot)
    print("-" * 72)
    print(f"[random] objective : mean = {mean:.1f} | best = {min(tot)} | "
          f"worst = {max(tot)} | sd = {var ** 0.5:.1f}")
    print(f"[random] extra splits : mean = {sum(spl)/len(spl):.1f} | "
          f"min = {min(spl)} | max = {max(spl)}")
    print(f"[random] pod visits   : mean = {sum(vis)/len(vis):.1f} | "
          f"min = {min(vis)} | max = {max(vis)}")
    print(f"[random] total wall time = {t_end - t_start:.1f} s "
          f"({REPLICATES} replications)")
